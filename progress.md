...
跨專案與工作區層級的待辦在 `D:\Codes\progress.md`（與本專案相關：X-1、X-6、X-11、X-13、X-16）。
...
- **#3** 〔決定〕`pyproject.toml` / `stable.toml` 的 Homepage 指向 `Intergration-Automation-Testing`，origin 已是 `JeffreyChen-SteamProjects`；改了會影響 PyPI 頁面（工作區 X-11）。
